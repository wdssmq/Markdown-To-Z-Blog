...
def http(req_arg=None, data_arg=None, return_format="data"):
    ...
    url = req_arg["base_url"] + "?mod=" + req_arg["mod"] + "&act=" + req_arg["act"]
...
def login():
    ...
    fnLog("## login")
    # 登录
    data_arg = {"username": config_info["API_USR"], "password": config_info["API_PWD"]}
    data = http({"method": "post", "mod": "member", "act": "login"}, data_arg)
...
def update_post(data_arg):
    ...
    data_arg["Intro"] = "请在正文内使用 &lt;!-- more --&gt;"
    data = http({"method": "post", "mod": "post", "act": "post"}, data_arg)
    if not data is None:
        post = data["post"]
        # print(post)
